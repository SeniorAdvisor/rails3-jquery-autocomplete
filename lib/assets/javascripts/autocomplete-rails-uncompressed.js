      ...
      jQuery(e).autocomplete({delay:jQuery(e).attr('data-autocomplete-delay'),
        source: function( request, response ) {
          ...
          jQuery.getJSON( jQuery(e).attr('data-autocomplete'), data, function() {
            ...
            // replace first argument object returned with value
            // {call_centers: [1,2,3]} // -> [1,2,3]
            var arg0Array = [];
            for (var key in args[0]) {
              if (args[0].hasOwnProperty(key) && $.isArray(args[0][key])) {
                arg0Array = args[0][key];
              }
            }
            args[0] = arg0Array;
            if(args[0].length == 0) {
              args[0] = []
              args[0][0] = { id: "", label: "no existing match" }
            }
